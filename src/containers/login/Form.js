import React, {useMemo} from 'react';
import {changeFormField as _changeFormField, selectLogin} from "../../redux/userSlice";
import {useDispatch, useSelector} from "react-redux";
// import useStyles from './recoveryPassword.style';
import {Button, TextField} from "@material-ui/core";

const changeFormField = (props) => _changeFormField({...props,form: 'login'});

//TODO: 计时时会重新渲染,TextField的问题
const Field = React.memo(({label, name, ...rest}) => {
  const {form, errors} = useSelector(selectLogin);
  // const classes = useStyles();
  const dispatch = useDispatch();

  const handleFormChange = React.useCallback((e) => {
    const {name, value} = e.target;
    dispatch(changeFormField({name, value}));
  }, [dispatch]);

  const value = useMemo(() => {
    return form[name];
  }, [form, name]);

  const error = useMemo(() => {
    const field = errors['name'];
    const value = errors['value'];
    return {
      text: value,
      isError: field === name
    };
  }, [errors, name]);

  return (
    <TextField
      name={name}
      value={value}
      onChange={handleFormChange}
      // className={classes.textField}
      label={label}
      color="primary"
      fullWidth={true}
      variant={"outlined"}
      error={error.isError}
      helperText={error.text}
      {...rest}
    />
  );
});


const SubmitBtn = React.memo(({children, handleOnSubmit, ...rest}) => {
  // const classes = useStyles();
  return (
    <Button
      // variant="contained"
      color="primary"
      onClick={handleOnSubmit}
      // className={classes.button}
      {...rest}
    >
      {children}
    </Button>
  );
});
export {Field, SubmitBtn};
